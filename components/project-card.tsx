...
function ProjectMedia({ project }: { project: Project }) {
    ...
    return (
      ...
      >
        ...
        <span className="absolute inset-0 flex items-center justify-center bg-black/20 transition-colors group-hover/play:bg-black/30">
          <span className="flex h-14 w-14 items-center justify-center rounded-full btn-bw-primary text-white shadow-lg transition-transform duration-200 group-hover/play:scale-110">
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-2 bg-white/5 text-muted-foreground">
      <ImageOff size={22} />
...
const statusStyles: Record<Project['status'], string> = {
  Completed: 'bg-white/6 text-foreground/60 border border-white/12',
...
export function ProjectCard({ project, onSelect }: ProjectCardProps) {
  return (
    <article
      ...
      className="glass group flex h-full flex-col overflow-hidden rounded-2xl transition-all duration-300 hover:-translate-y-1.5 hover:border-white/25 hover:shadow-xl hover:shadow-black/30 cursor-pointer"
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="text-lg font-semibold text-foreground group-hover:text-foreground/60 transition-colors">
          {project.title}
        </h3>
        ...
        <ul className="mt-4 flex flex-wrap gap-1.5">
          {project.technologies.map((tech) => (
            <li
              key={tech}
              className="rounded-md border border-white/12 bg-white/5 px-2 py-0.5 font-mono text-[11px] text-foreground/70"
            >
              {tech}
            </li>
          ))}
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-2 border-t border-border/60 pt-4">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg btn-bw-primary px-3 py-2 text-xs font-semibold text-white shadow-md transition-transform duration-200 hover:scale-105"
          >
